...
function renderSource(item) {
  ...
  const identity = node("div", "record-title");
  identity.append(node("strong", "record-primary", `${item.flag || "◌"} ${item.ip}`), node("span", "record-secondary", `${item.city}, ${item.country} · ${item.asn ? `AS${item.asn}` : "ASN unresolved"} · ${item.organization}`));
  const facts = node("dl", "record-facts"); [["Sessions", item.sessions], ["Auth", item.auth_attempts], ["Accepted", item.accepted], ["Commands", item.commands], ["Artifacts", item.downloads], ["Protocols", (item.protocols || []).join(", ")], ["First", formatDate(item.first_seen)], ["Last", formatDate(item.last_seen)]].forEach(([label, value]) => { const row = node("div"); row.append(node("dt", "", label), node("dd", "", String(value))); facts.append(row); }); article.append(identity, facts); return article;
}
...
function artifactCategory(item) { const correlation = item.correlation, providers = correlation?.providers || []; if (providers.some((provider) => provider.status === "correlated" && (provider.name !== "VirusTotal" || Number(provider.malicious || 0) > 0))) return "Provider-detected"; if (correlation?.status === "correlated") return "Provider-observed"; if (correlation?.status === "not-found") return "No provider record"; if (correlation?.status === "partial") return "Partial provider coverage"; if (item.classification?.status === "known") return "Provider-detected"; if (item.classification?.status === "unknown") return "No provider record"; return "Correlation unavailable"; }
function renderArtifact(item) { const article = node("article", "evidence-record artifact-record"); article.append(node("code", "record-primary", item.url || "Malformed transfer reference withheld"), node("code", "record-hash", item.sha256 || "SHA-256 unavailable")); const facts = node("dl", "record-facts"); [["Count", item.count], ["First", formatDate(item.first_seen)], ["Last", formatDate(item.last_seen)], ["ATT&CK", (item.techniques || []).join(", ")], ["Category", artifactCategory(item)]].forEach(([label, value]) => { const row = node("div"); row.append(node("dt", "", label), node("dd", "", String(value))); facts.append(row); }); article.append(facts); const providers = node("div", "provider-list"); (item.correlation?.providers || []).forEach((provider) => { const card = node("div", "provider-card"); card.append(node("strong", "", provider.name), node("span", "", provider.label || provider.status)); if (provider.name === "VirusTotal" && provider.malicious !== null) card.append(node("small", "", `${provider.malicious} malicious · ${provider.suspicious} suspicious · ${provider.harmless} harmless · ${provider.undetected} undetected`)); if (provider.retrieved_at) card.append(node("time", "", formatDate(provider.retrieved_at))); if (provider.report_url) { const link = node("a", "", "Open provider report ↗"); link.href = provider.report_url; link.target = "_blank"; link.rel = "noopener noreferrer"; card.append(link); } providers.append(card); }); if (!providers.childNodes.length) providers.append(node("p", "empty-state", "No provider correlation is available for this hash.")); article.append(providers); return article; }
...
async function load() { try { const response = await fetch(`${DATA_URL}?v=${Date.now()}`, { cache: "no-store" }); if (!response.ok) throw new Error(`telemetry request returned HTTP ${response.status}`); snapshot = await response.json(); if (!["3.0", "4.0", "5.0"].includes(snapshot.schema_version)) throw new Error(`unsupported telemetry schema ${snapshot.schema_version}`); byId("evidence-generated").textContent = formatDate(snapshot.generated_at); byId("evidence-schema").textContent = snapshot.schema_version; byId("evidence-window").textContent = `${formatDate(snapshot.window?.start)} — ${formatDate(snapshot.window?.end)}`; render(); } catch (error) { const banner = byId("evidence-error"); banner.textContent = `Evidence snapshot unavailable: ${error.message}`; banner.hidden = false; } }
